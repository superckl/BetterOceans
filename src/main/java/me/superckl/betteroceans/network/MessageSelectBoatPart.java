...
public class MessageSelectBoatPart implements IMessage{
	...
	public void fromBytes(final ByteBuf buf) {
		World world = WorldProvider.getProviderForDimension(buf.readInt()).worldObj;
		if(world == null){
			LogHelper.error("Failed to deserialize TileEntityBoatWorkbench!");
			return;
		}
		TileEntity te = world.getTileEntity(buf.readInt(), buf.readInt(), buf.readInt());
		if(te == null || te instanceof TileEntityBoatWorkbench == false){
			LogHelper.error("Failed to deserialize TileEntityBoatWorkbench!");
			return;
		}
		this.te = (TileEntityBoatWorkbench) te;
		BoatPart part = BoatHelper.readPartFromBuffer(buf);
		if(part == null){
